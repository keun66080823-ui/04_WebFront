...
btn1.addEventListener("click", function(){
  ...
  if(value.length == 0) {
    ...
    return; // 함수 종료 호출 + 호출한 곳으로 되돌아감
  }

  let result; // 입력된 값이 0 / 양수 / 음수인지 판단 및 저장
  ...
  // 중요!! 전달 인자의 값의 순서가 중요합니다!
  print1(result, value); // 함수 호출
...
document.querySelector("#btn2a").addEventListener("click", function(){
  ...
  const arr = ["사과", "바나나", "딸기"];
  let x = 0; // 원시 타입 -> 참조형이 아니기에 주소값이 아닌 값 자체를 가지고 있음 
...
arrowList[1].addEventListener("click", e => {
  //e.target.style.backgroundColor = "darkred"; // e.target은 현재 이벤트가 일어난 대상을 지칭한다. 즉, arrowList[1]를 가리킨다.
  //this.EventTarget.style.backgroundColor = "darkred"; // 에러가 남
  // 화살표 함수에서의 this
  // 이벤트가 일어난 대상 객체가 아니라
  // 전역객체 (window)
  console.log(this);
...
// return 한 줄만 작성된 경우 -> 리턴 구문과 중괄호 생략 가능
