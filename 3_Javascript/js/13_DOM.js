...
.addEventListener("click", () => {
  ...
  // 3. 마지막 자식 노드 찾기(lastChild)
  console.log( ul.lastChild );

  // 4. 원하는 위치(index)에 존재하는 자식 노드 찾기
  // (요소.childNodes[index])
  // ul의 자식 노드 중 3번째 li 태그를 찾아서 배경색 darkred 지정
  ul.childNodes[9].style.backgroundColor = "darkred";
  ...
  // ul.chlidNode[9]의 다음 다음 형제 노드 선택
  // == ul.childNodes[11]
  ...
  // 3. 첫 번쩨 자식으로 추가 ( 요소.prepend("내용" or 노드) )
...
document.querySelector("#btn2").addEventListener("click", () => {
  ...
  // ul의 모든 자식 요소 얻어오기 (노드가 아니라 요소. children)
