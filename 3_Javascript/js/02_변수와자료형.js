...
// 2. let (변수, var의 변수명 중복 문제를 해결해준 키워드.)
...
// const test; 상수는 선언과 동시에 반드시 초기화하기!
...
console.log(foo1); // 전역변수이기 때문에 (블록과 관계 없음) 접근 가능
console.log(foo2); // 전역변수이기 때문에 (블록과 관계 없음) 접근 가능
// console.log(num1);
// console.log(num2);
...
function example2() { // 함수 블록(함수레벨 scope)
...
// 에러가 안 나고 undefined(변수값이 없음) 가 찍힘.
...
// -> 초기화기 이루어지기 전까지 해당 변수는 TDZ에 놓이며,
// 이 기간동안 변수에 접근하려고 하면
// Cannot access 'x' before initialization 라는 에러 발생.

// console.log(x); 
// ReferenceError: Cannot access 'x' before initialization (초기화 전에 접근할 수는 없다)
...
//  예측 가능한 코드를 작성할 수 있음 (오늘날 let/const로 쓰는 걸 지향.)
...
const gender = 'M'; // 홑따옴표에 한 글자만 작성해도 char가 아닌 문자열(string)이다.
...
console.log("age : ", eyesight, typeof eyesight);
