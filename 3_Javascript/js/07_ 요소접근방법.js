...
function classTest() {
  ...
  // 제공하지 않는 기능이 : pop(), push(), map(), filter() 등...
  // HTMLCollection, NodeList는 유사배열.
...
function readValue() {
  ...
  bg.scrollTop = bg.scrollHeight; // 입력한 최신 메세지에 따라서 스크롤이 내려옴.
...
.addEventListener("keyup", function(e) {
  ...
  // e['key'] -> 매핑된 value
  // e.key -> 매핑된 value
  if(e.key == "Enter") { // 엔터가 눌려지고 떼어졌을 때
    readValue();
  }
});