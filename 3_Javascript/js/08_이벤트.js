...
function check1(btn) {
  ...
  // 버튼이 클릭 될 때마다 yellow색이라면 -> pink. pink색이라면 -> yellow.
  if(bgColor == "yellow"){
...
// 이벤트 핸들러 제거하기
document.querySelector("#test1-2")
.onclick = function() {
...
// -> onclick에 저장된 값(이벤트핸들러 함수)이
// 덮어씌워지면서 이전코드(배경색 red)가
// 사라지는 문제 발생 (정리하자면, 중복되는 이벤트리스너일 경우 중첩되지 않고 최근 것만 살아남는다.) 
...
test2.addEventListener("click", function(){
  ...
  // == 이벤트가 발생한 요소. 즉 이 함수에서 this는 test2가 된다.
